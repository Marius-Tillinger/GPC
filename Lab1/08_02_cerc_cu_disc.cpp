...
// ================================================
// | Laboratorul VIII - 08_02_cerc_cu_disc.cpp |
// ===========================================
// 
//	Program ce deseneaza un cerc cu un disc cu ajutorul reprezentarii parametrice, folosindu-se tehnicile MODERN OpenGL;
//  ELEMENTE DE NOUTATE:
...
float cx = 10.f, cy = 5.f, radius = 20.0, a = 10.0f;
...
void CreateVBO(void)
{
	...
	for (int i = 0; i < NR_POINTS; i++) // sunt NR_POINTS varfuri
	{
		...
		vf_coord[i] =
			glm::vec4(
				cx + a * pow(cos(2 * PI * i / NR_POINTS), 3),
				cy + a * pow(sin(2 * PI * i / NR_POINTS), 3),
				0.0f, 1.0f);
	};
...
int main(int argc, char* argv[])
{
	...
	glutCreateWindow("Cerc cu disc - reprezentare parametrica - OpenGL <<nou>>");		//	Creeaza fereastra de vizualizare, indicand numele acesteia;
